...
const NewTendencies = () => {
  ...
  const [beatsPastTwoYears, setBeatsPastTwoYears] = useState(0);
  const [missesPastTwoYears, setMissesPastTwoYears] = useState(0);
  const [sameEventRaisesPastThree, setSameEventRaisesPastThree] = useState(0);
  const [sameEventCutsPastThree, setSameEventCutsPastThree] = useState(0);
  ...
  const tendenciesSubmitHandler = async event => {
    ...
    try {
      await sendRequest(
        process.env.REACT_APP_BACKEND_URL + '/tendencies',
        'POST',
        JSON.stringify({
